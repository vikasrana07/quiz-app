...
export class AuthenticationService {
  ...
  generateToken(username: string, password: string): Observable<User> {
    ...
    return this.httpService.post('auth/generatetoken', params).pipe(
      map((user: User) => {
  ...
  validateToken(): Observable<User> {
    return this.httpService.get('auth/validatetoken').pipe(
      map((user: User) => {
        return user;
      })
    );
  ...
  isLoggedIn() {
    ...
    if (user?.isLoggedIn) this.isLogin = true;
    else this.isLogin = false;
    return this.isLogin;
  }

  logout(): Observable<any> {
    return this.httpService.delete('auth/logout').pipe(
      map((response) => {
        this.clearCache();
        return response;
      })
